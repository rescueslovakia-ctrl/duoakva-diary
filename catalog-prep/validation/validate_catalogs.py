...
for filename,keycols in [
    ...
    for i,r in enumerate(data,2):
        key=' | '.join((r.get(c) or '').strip().lower() for c in keycols)
        keys.append(key)
        status=(r.get('verification_status') or '').strip()
        if status=='verified' and not (r.get('source_url') or '').strip():
            errors.append(f'{filename}:{i}: verified row without source_url')
        if filename in ('plants.csv','livestock.csv'):
            for a,b,label in [('temperature_min','temperature_max','temperature'),('ph_min','ph_max','pH'),('gh_min','gh_max','GH'),('kh_min','kh_max','KH')]:
                check_range(r,a,b,label,f'{filename}:{i}')
...
nutrients=rows('fertilizer_nutrients.csv')
verified_nutrients=Counter()
...
for key,r in fert.items():
    if (r.get('calculation_safe') or '').strip().lower() in ('true','1','yes') and verified_nutrients[key]==0:
        errors.append(f'fertilizers.csv: calculation_safe without verified nutrient evidence: {key}')
