...
export default function FertilizingSection({aquariums}:{aquariums:Aquarium[]}){const[tab,setTab]=useState<Tab>('correction');return <div className="fertilizing-v1"><div className="fertilizing-v1-intro"><div><small>HNOJENIE AKVÁRIA</small><h3>Hnojenie a úprava vody</h3><p>Korekčné dávkovanie podľa aktuálnych meraní, pravidelný režim a mineralizácia vody sú oddelené, aby bolo vždy jasné, podľa čoho aplikácia odporúčanie vytvorila.</p></div><ShieldCheck size={24}/></div><div className="fertilizing-v1-tabs"><button type="button" className={tab==='correction'?'active':''} onClick={()=>setTab('correction')}><Droplets size={16}/> Korekcia podľa merania</button><button type="button" className={tab==='schedule'?'active':''} onClick={()=>setTab('schedule')}><CalendarDays size={16}/> Pravidelný režim</button><button type="button" className={tab==='water'?'active':''} onClick={()=>setTab('water')}><Waves size={16}/> Úprava vody / mineralizácia</button></div><div className={`fertilizing-v1-content fertilizing-${tab}`}>{tab==='correction'?<><MeasurementFreshnessNotice aquariums={aquariums}/><FertilizerMeasurementGuard aquariums={aquariums}/><FertilizerUserDataPanel aquariums={aquariums}/><FertilizersModule aquariums={aquariums}/></>:tab==='schedule'?<RegularFertilizerSchedule aquariums={aquariums}/>:<WaterTreatmentModule aquariums={aquariums}/>}</div></div>}
