...
type Point={measured_at:string;timestamp:number;[key:string]:string|number|null};
...
function MeasurementTrendChart({aquariums}:{aquariums:Aquarium[]}){
 ...
 return <>
  <section className="card">
   ...
   <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",margin:"2px 0 14px"}}><button type="button" className={showLivestockEvents?"primary":""} onClick={()=>setShowLivestockEvents(v=>!v)}><Fish size={15}/> Udalosti osádky</button><span className="muted">Voliteľne zobrazí dátumy pridania živočíchov priamo v grafe.</span></div>
   {showLivestockEvents&&visibleEvents.length>0&&<div className="notice" style={{marginBottom:14}}><b>Udalosti v zobrazenom období</b><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}>{visibleEvents.map(e=><span key={e.id} className="status-pill status-neutral">🐟 {new Date(`${e.added_date}T12:00:00`).toLocaleDateString('sk-SK')} · {e.quantity}× {e.name}</span>)}</div><div className="muted" style={{marginTop:8}}>Značky ukazujú časovú súvislosť s vývojom parametrov, nie automaticky príčinu ich zmeny.</div></div>}
   ...
   {loading?<div className="notice">Načítavam graf…</div>:visibleSelected.length?<div style={{width:"100%",height:390}}><ResponsiveContainer><LineChart data={chartData}><CartesianGrid strokeDasharray="3 5" vertical={false}/><XAxis dataKey="timestamp" type="number" scale="time" domain={["dataMin","dataMax"]} tickFormatter={v=>new Date(Number(v)).toLocaleDateString("sk-SK",{day:"2-digit",month:"2-digit"})}/><YAxis domain={multi?[0,100]:["auto","auto"]} tickFormatter={v=>multi?`${Math.round(v)}%`:fmt(Number(v))} label={oneInfo?.unit?{value:oneInfo.unit,angle:-90,position:"insideLeft"}:undefined}/><Tooltip content={<TrendTooltip selected={selected} multi={multi}/>}/><Legend formatter={value=>fieldInfo(String(value)).label}/>{!multi&&target?.min!==undefined&&target?.max!==undefined&&<ReferenceArea y1={target.min} y2={target.max} fill="#22c55e" fillOpacity={.08}/>} {visibleEvents.map(e=><ReferenceLine key={`line-${e.id}`} x={e.timestamp} stroke="#d97706" strokeDasharray="4 4" strokeOpacity={.7} label={{value:`+${e.quantity} osádka`,position:"insideTopRight",fill:"#b45309",fontSize:11}}/>)}{visibleEvents.flatMap(e=>visibleSelected.map(code=>{const y=interpolateAt(chartData,code,e.timestamp);return y==null?null:<ReferenceDot key={`dot-${e.id}-${code}`} x={e.timestamp} y={y} r={5} fill="#f59e0b" stroke="#ffffff" strokeWidth={2} ifOverflow="discard"/>}))}{selected.map((code,i)=><Line key={code} type="monotone" dataKey={code} connectNulls stroke={colors[i%colors.length]} strokeWidth={3} dot={{r:3}} name={code}/>)}</LineChart></ResponsiveContainer></div>:<div className="notice">Pre zvolené obdobie nie sú dostupné hodnoty vybraných parametrov.</div>}
  </section>
