...
export default function LivestockDiscovery({name,category,onUse}:{name:string;category:string;onUse:(result:DiscoveryResult)=>void}){
  ...
  async function discover(){
    const q=name.trim();
    if(q.length<3){setError("Zadaj aspoň 3 znaky názvu živočícha.");return}
    ...
    try{
      ...
      const list=Array.isArray(data)?data:Array.isArray(data?.items)?data.items:Array.isArray(data?.results)?data.results:data?.result?[data.result]:[];
      setResults(list);
      if(!list.length)setError(data?.message||"Nenašli sa dostatočne spoľahlivé údaje. Skús presnejší slovenský alebo vedecký názov.");
    }catch(e:any){setError(e?.message||"Online dohľadanie sa nepodarilo.")}
  ...
  return <div style={{marginTop:12}}>
    <button type="button" onClick={discover} disabled={loading||name.trim().length<3}><Search size={16}/> {loading?"Dohľadávam…":"Dohľadať údaje online"}</button>
    {error&&<p className="muted" style={{marginTop:8}}>{error}</p>}
    {results.map((x,i)=><div key={`${x.scientific_name||x.common_name||i}-${i}`} className="catalog-result" style={{marginTop:10,alignItems:"flex-start"}}>
      <div style={{flex:1}}>
        ...
        <p className="muted">Údaje nájdené online; uložia sa až po tvojom potvrdení.</p>
      </div>
