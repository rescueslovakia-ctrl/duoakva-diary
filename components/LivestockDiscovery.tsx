...
export default function LivestockDiscovery({name,category,existing,onUse}:Props){
 ...
 async function discover(){const q=name.trim();if(q.length<3){setError("Zadaj aspoň 3 znaky názvu živočícha.");return}setLoading(true);setError("");setResults([]);try{const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),10000);const r=await fetch(`/api/livestock-discovery?q=${encodeURIComponent(q)}&category=${encodeURIComponent(category)}`,{signal:controller.signal});clearTimeout(timeout);const data=await r.json();if(!r.ok)throw new Error(data?.error||"Online dohľadanie sa nepodarilo.");const raw=Array.isArray(data)?data:Array.isArray(data?.items)?data.items:Array.isArray(data?.results)?data.results:data?.result?[data.result]:[];const list=(raw as DiscoveryResult[]).filter(x=>category==="other"||!x.category||x.category===category);setResults(list);if(!list.length)setError(data?.message||`Nenašla sa spoľahlivá zhoda v kategórii ${categoryLabels[category]||"živočích"}. Skús vedecký alebo iný bežný názov.`)}catch(e:any){setError(e?.name==="AbortError"?"Dohľadanie trvalo príliš dlho. Skús to znova.":e?.message||"Online dohľadanie sa nepodarilo.")}finally{setLoading(false)}}
 ...
 return <div style={{marginTop:12}}>
  ...
  {results.map((x,i)=>{const merged=mergeMissing(existing,x),before=missing(existing),after=missing(merged),added=Math.max(0,before-after);return <div key={`${x.scientific_name||x.common_name||i}-${i}`} className="catalog-result" style={{marginTop:10,alignItems:"flex-start"}}><div style={{flex:1}}>
   ...
   <p className="muted">{categoryLabels[x.category||category]||"Živočích"} · {x.match_label||"Online zhoda"}</p>
