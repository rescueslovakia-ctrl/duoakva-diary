...
export default function PlantsModule({aquariums}:{aquariums:Aquarium[]}){
  ...
  const lightSummary=useMemo(()=>{
    ...
    const currentWpl=aq&&aq.net_volume_l?effectiveW/aq.net_volume_l:0;
    const fullWpl=aq&&aq.net_volume_l?nominalW/aq.net_volume_l:0;
    ...
    const currentAdjusted=currentWpl*heightPenalty;
    const fullAdjusted=fullWpl*heightPenalty;
    const currentLevel=currentAdjusted>=.35?3:currentAdjusted>=.2?2:currentAdjusted>0?1:0;
    const fullLevel=fullAdjusted>=.35?3:fullAdjusted>=.2?2:fullAdjusted>0?1:0;
    const limitedLights=lights.filter(x=>Number(x.settings?.intensity_pct??100)<100);
    const weightedIntensity=nominalW>0?lights.reduce((sum,x)=>sum+Number(x.specs?.light_w||x.specs?.power_w||0)*Number(x.settings?.intensity_pct??100),0)/nominalW:100;
    return {count:lights.length,nominalW,effectiveW,currentWpl,fullWpl,currentAdjusted,fullAdjusted,currentLevel,fullLevel,spectra,photoperiod,heightCm:aq?.height_cm,limitedLights,weightedIntensity};
  ...
  const lightOk=plantRequirement.light===0||lightSummary.currentLevel>=plantRequirement.light;
  ...
  const dimmed=lightSummary.limitedLights.length>0;
  ...
  return <>
    ...
    <section className="card"><h3>Vyhodnotenie rastlín a techniky</h3><p><b>Aktuálne osvetlenie:</b> {lightSummary.count} svetiel · nominálne {lightSummary.nominalW.toFixed(1)} W · po intenzite ≈ {lightSummary.effectiveW.toFixed(1)} W · {lightSummary.currentWpl.toFixed(2)} efektívneho W/l{lightSummary.spectra.length?` · spektrá ${lightSummary.spectra.join(", ")}`:""}</p><p><b>Fotoperióda:</b> {lightSummary.count?`${lightSummary.photoperiod.toFixed(1)} h/deň (vážený priemer podľa výkonu)`:"nezadaná"}{lightSummary.heightCm?` · výška nádrže ${lightSummary.heightCm} cm`:""}</p><p><b>CO₂:</b> {hasCo2?"systém evidovaný":"bez evidovaného CO₂ systému"}</p>{dimmed&&<div className="notice"><b>⚠️ Osvetlenie nejde na maximálny výkon.</b><p>Výkonovo vážená intenzita celej zostavy je približne <b>{lightSummary.weightedIntensity.toFixed(0)} %</b>. Pri aktuálnom nastavení vychádza osvetlenie orientačne pre <b>{levelLabel(lightSummary.currentLevel)}</b>.</p><p>Pri nastavení všetkých svetiel na 100 % by zostava mala približne {lightSummary.fullWpl.toFixed(2)} W/l pred korekciou výšky a orientačne by zodpovedala kategórii <b>{levelLabel(lightSummary.fullLevel)}</b>.</p>{lightSummary.fullLevel>lightSummary.currentLevel&&<p>Zvýšenie výkonu teda môže rozšíriť výber rastlín z kategórie „{levelLabel(lightSummary.currentLevel)}“ na „{levelLabel(lightSummary.fullLevel)}“. Vyššia intenzita však môže zvýšiť aj nároky na stabilné CO₂ a hnojenie a pri nevyváženej nádrži podporiť riasy.</p>}</div>}{items.length?<p>{lightOk&&co2Ok&&!photoWarning?"✅ Aktuálna kombinácia techniky orientačne zodpovedá náročnosti evidovaných rastlín.":`⚠️ ${!lightOk?`Celkové osvetlenie môže byť pri aktuálnej intenzite slabé pre najnáročnejšiu rastlinu${plantRequirement.hardest?` (${plantRequirement.hardest})`:""}. ${dimmed&&lightSummary.fullLevel>=plantRequirement.light?"Pri 100 % výkone by svetelný potenciál tejto požiadavke orientačne zodpovedal. ":""}`:""}${!co2Ok?"Niektoré rastliny odporúčajú alebo vyžadujú CO₂. ":""}${photoWarning?"Fotoperióda je mimo bežného orientačného rozsahu 5–10 hodín/deň.":""}`}</p>:<p className="muted">Po pridaní rastlín sa tu zobrazí spoločné vyhodnotenie.</p>}<p className="muted">W/l ostáva orientačný údaj. Hodnotenie už zohľadňuje nastavenú intenzitu svetiel, výšku nádrže a fotoperiódu. Keď bude pri konkrétnom svetle dostupný PAR, bude mať pri hodnotení vyššiu váhu než W/l.</p></section>
